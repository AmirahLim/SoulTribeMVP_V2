...
function PeopleListContent() {
  ...
  // The thrown error is kept, not flattened to a string, so its type can still
  // distinguish a missing location from a genuine failure.
  ...
  }, [authUser?.id, city, reloadTrigger]);

  const matchListState = resolveMatchListState({ loading, error, matchCount: matches.length, spatialPoolSize });
  ...
  return (
    <div className="relative min-h-screen w-full bg-black text-[#FFFDF9] pb-24">
      ...
      <div className="relative z-10 mx-auto max-w-[440px] px-5 pt-8">
        ...
        {matchListState.kind === 'list' && isSmallCommunity && (
          <motion.div
        ...
        {/* Location unavailable, nobody in range, nobody eligible and load failure
            each get their own words. None of them renders an empty match list. */}
        {matchListState.kind !== 'loading' && matchListState.kind !== 'list' && (
