...
export function activityKey(category:unknown):string{
...
export function scoreCacheRow(input:{
  viewerId:string;candidateId:string;activity:string;
...
export async function readMatchScores(
  client:SupabaseClient,viewerId:string,activity:string,candidateIds:string[],
...
export async function writeMatchScores(client:SupabaseClient,rows:ReturnType<typeof scoreCacheRow>[]):Promise<void>{
  ...
  const {error}=await client.from('match_scores').upsert(rows,{onConflict:'user_a,activity_key,user_b'});
